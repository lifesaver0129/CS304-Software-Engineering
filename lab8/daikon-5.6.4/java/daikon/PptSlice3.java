...
public final class PptSlice3 extends PptSlice {
  ...
  public final int arity(
      /*>>>@UnknownInitialization(PptSlice.class) @Raw(PptSlice.class) PptSlice3 this*/) {
    return 3;
  }
  /**
  ...
  protected PptSlice cloneAndPivot(VarInfo[] argNewVarInfos) {
    ...
    List<Invariant> newInvs = new ArrayList<Invariant>();
    for (Invariant inv : invs) {
      assert inv.ppt == this;
  ...
  public void merge_invariants() {
    ...
    for (PptRelation rel : parent.children) {
      ...
      if (slice_missing(ppt, cvis)) {
        if (debugMerge.isLoggable(Level.FINE)) {
          debugMerge.fine(
              "-- slice ignored (missing) " + ppt.name() + " vars " + VarInfo.arrayToString(cvis_sorted));
        }
        continue;
      }
      ...
      if ((cslice == null) || (cslice.invs.size() == 0)) {
        ...
        if (debugMerge.isLoggable(Level.FINE)) {
          debugMerge.fine(
              "-- slice not found " + ppt.name() + " vars " + VarInfo.arrayToString(cvis_sorted));
        }
        return;
      }
      ...
      if (debugMerge.isLoggable(Level.FINE)) {
        ...
        debugMerge.fine("-- -- child vars = " + VarInfo.arrayToString(cvis_sorted));
        debugMerge.fine("-- -- parent vars = " + VarInfo.arrayToString(var_infos));
    ...
    Map<Invariant.Match, List<Invariant>> inv_map =
        new LinkedHashMap<Invariant.Match, List<Invariant>>();
