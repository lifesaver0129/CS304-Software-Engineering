    ...
    private void calculateSuspiciousnessAndConfidence() {
        ...
        for (int i = 0; i < numStmts; i++) {
            if ((totalLiveFail == 0) && (totalLivePass == 0)) {
                // total number of passed tests or failed tests
                suspiciousness[i] = -1d;
                confidence[i] = -1d;
                ...
                // You should only care about the value here
                // Tarantula:
                suspiciousness[i] = passRatio[i] / (failRatio[i] + passRatio[i]);
                // Kulczynski
                // suspiciousness[i] = failRatio[i] / (failRatio[i] + passRatio[i]);
                // DStar2
                // suspiciousness[i] = failRatio[i]*failRatio[i] / (failRatio[i] + passRatio[i]);
                // Wong2
                //suspiciousness[i] = passRatio[i] > failRatio[i] ? passRatio[i] - failRatio[i]: failRatio[i] - passRatio[i];
